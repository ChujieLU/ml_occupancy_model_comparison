...
METHOD_NAME = "LR"
SAVE_PATH = os.path.join(os.path.expanduser("~"),"Documents/PhD_Work/home_occupancy_state",METHOD_NAME,"results_where")
...
# def increase_values(tstamp, row):
#     row.at['H_t'] = row.loc['H_t'] + 1
#     tstamp = tstamp + dt.timedelta(minutes=30)
#     if row['H_t'] > 47:
#         row.at['W_t'] = util.Is_This_A_Weekday(tstamp)
#         row.at['H_t'] = 0
#     return row
...
class lr_test(object):
    ...
    def Test_Data_n_steps(self, feature_order, n_steps=1):
        ...
        for row_i, data in self.test.iterrows():
            ...
            #if n_steps ==0:
            #    print(data.filter(feature_order), pred_)
